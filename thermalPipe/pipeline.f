...
C     dS/dx = mu
C     dU/dx = mu*cos(theta) - 1
C     dW/dx = mu*sin(theta)
C     
C     * equilibrium equantions:
C     dH/dx = -mu*q_x
C     dV/dx = -mu*q_y
C     dM/dx = mu*(Hsin(theta) - Vcos(theta))
C     qx = 0
C     qy = -k1*W - k2*W^3
C
C     * constitutive equantions:
C     N = EA(mu - 1 - alpha*T)
C     M = -EI/mu * dtheta/dx
C     N = Hcos(theta) + Vsin(theta)
C     mu = [(Hcos(theta) + Vsin(theta))/EA] + alpha*T + 1
C
C     * unkonwns
C     S,U,W,theta,H,V,M
C
C     - nondimensional quantities:
C     (xi,s,u,w) = (x,S,U,W)/l
C     lamada = l(A/I)^0.5
C     tao = alpha*lamada^2*T
C     (k_1, k_2) = (k1, k2)*l^4/EI
C     (P_h, P_v) = (H, V)*l^2/EI
C     m = lM/EI
C     
C     - ODEs(dementionless)(7 equations):
C     ds = mu
C     du = mu*cos(theta) - 1
C     dw = mu*sin(theta)
C     dtheta =  -m/mu
C     dm = mu*(P_h*sin(theta) - P_v*cos(theta))
C     dP_h = mu*u*(k_1 + k_2*(u^2 + w^2))
C     dP_v = mu*w*(k_1 + k_2*(u^2 + w^2))
C
C     * unknowns
C     s,u,w,theta,m,P_h,P_v
C
C     - Parameters:
C     mu = (P_h*cos(theta) + P_h*sin*(theta) + tao)/lamada^2 + 1
C
C     
C     EI     bending stiffness
C     A      section area
C     T      Temperature
C     alpha  Expension coeffcient
C
