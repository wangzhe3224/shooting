...
!  parameters:
      ...
      f(2)= (u(4)-dcos(u(1)))/dsin(u(1))/dsin(u(1))-&                ! psi
             vv*(1d0+xlamda*dcos(u(1)))*&
           dsin(u(1))*dcos(u(2))/dsqrt(xmu-2d0*vv*u(4))&
           -xlamda*vv/xalpha*dsin(u(1))*dcos(u(2))*&
            dsin(u(1))*dcos(u(2))&
           -xepirou

      f(3)= -(u(4)-dcos(u(1)))*(1d0-u(4)*dcos(u(1)))/dsin(u(1))&     ! p_theta
              /dsin(u(1))/dsin(u(1))&
           +xalpha*dsin(u(1))*(1d0+xlamda*dcos(u(1)))&
             -(dcos(u(1))+xlamda*(dcos(u(1))*dcos(u(1))&
              -dsin(u(1))*dsin(u(1))))*&
            dcos(u(2))*dsqrt(xmu-2d0*vv*u(4))&
            -xlamda/xalpha*dsin(u(1))*dcos(u(1))*dcos(u(2))*&
            dcos(u(2))*(xmu-2d0*vv*u(4))

      f(4)= (1d0+xlamda*dcos(u(1)))*dsin(u(1))*dsin(u(2))*&          ! p_psi
              dsqrt(xmu-2d0*vv*u(4))+&
             xlamda/xalpha*dsin(u(2))*(xmu-2d0*vv*u(4))*&
...
!
       READ(10,*) xepirou,xlamda,vv,xalpha,xmu
...
! NOW DEFINE THE BOUNDARY CONDITIONS
      ...
      FB(2)= (fp4-dcos(fp1))/dsin(fp1)/dsin(fp1)-&
             vv*(1d0+xlamda*dcos(fp1))*&
           dsin(fp1)*dcos(fp2)/dsqrt(xmu-2d0*vv*fp4)&
           -xlamda*vv/xalpha*dsin(fp1)*dcos(fp2)*&
            dsin(fp1)*dcos(fp2)&
           -xepirou

      FB(3)= -(fp4-dcos(fp1))*(1d0-fp4*dcos(fp1))/dsin(fp1)&
              /dsin(fp1)/dsin(fp1)&
           +xalpha*dsin(fp1)*(1d0+xlamda*dcos(fp1))&
             -(dcos(fp1)+xlamda*(dcos(fp1)*dcos(fp1)&
              -dsin(fp1)*dsin(fp1)))*&
            dcos(fp2)*dsqrt(xmu-2d0*vv*fp4)&
            -xlamda/xalpha*dsin(fp1)*dcos(fp1)*dcos(fp2)*&
            dcos(fp2)*(xmu-2d0*vv*fp4)

      FB(4)= (1d0+xlamda*dcos(fp1))*dsin(fp1)*dsin(fp2)*&
              dsqrt(xmu-2d0*vv*fp4)+&
             xlamda/xalpha*dsin(fp2)*(xmu-2d0*vv*fp4)*&
              dsin(fp1)*dsin(fp1)*dcos(fp2)
...
!
      eq2= (U4-dcos(U1))/dsin(U1)/dsin(U1)-&
           ...
           dsin(U1)*dcos(U2)/dsqrt(xmu-2d0*vv*U4)&
      ...
      eq3= -(U4-dcos(U1))*(1d0-U4*dcos(U1))/dsin(U1)&
           ...
           +xalpha*dsin(U1)*(1d0+xlamda*dcos(U1))&
             -(dcos(U1)+xlamda*(dcos(U1)*dcos(U1)&
              -dsin(U1)*dsin(U1)))*&
            dcos(U2)*dsqrt(xmu-2d0*vv*U4)&
            -xlamda/xalpha*dsin(U1)*dcos(U1)*dcos(U2)*&
            dcos(U2)*(xmu-2d0*vv*U4)
!
      eq4= (1d0+xlamda*dcos(U1))*dsin(U1)*dsin(U2)*&
              dsqrt(xmu-2d0*vv*U4)+&
             xlamda/xalpha*dsin(U2)*(xmu-2d0*vv*U4)*&
...
!****************************************************************************
      ...
      common /init/xl,delta
        ...
        a(2,1)=1d0/Ss1-2d0*Sc1*(Sp-Sc1)/Ss1/Ss1/Ss1&
             -(vv*Sc1*Sc2*(1d0+xlamda*Sc1)&
               -xlamda*vv*Ss1*Ss1*Sc2)/&
              dsqrt(xmu-2d0*vv*Sp)&
             -2d0*xlamda*vv*Ss1*Sc1*Sc2*Sc2/xalpha

        a(2,2)=vv*(1d0+xlamda*Sc1)*Ss1*Ss2/dsqrt(xmu-2d0*vv*Sp)&
              +2d0*xlamda*vv*Ss1*Ss1*Ss2*Sc2/xalpha

        a(2,3)=0d0

        a(2,4)=1d0/Ss1/Ss1-vv*vv*(1d0+xlamda*Sc1)*Ss1*Sc2/&
             dsqrt((xmu-2d0*vv*Sp)*(xmu-2d0*vv*Sp)*(xmu-2d0*vv*Sp))

        a(3,1)=3d0*Sc1*(Sp-Sc1)*(1d0-Sp*Sc1)/Ss1/Ss1/Ss1/Ss1&
              -(1d0-2d0*Sp*Sc1+Sp*Sp)/Ss1/Ss1+xalpha*Sc1*(1d0+xlamda*Sc1)&
              -xalpha*xlamda*Ss1*Ss1+(Ss1+4d0*xlamda*Ss1*Sc1)*Sc2*&
              dsqrt(xmu-2d0*vv*Sp)+xlamda*(Ss1*Ss1-Sc1*Sc1)*Sc2*Sc2*&
              (xmu-2d0*vv*Sp)/xalpha

        a(3,2)=Ss2*(Sc1+xlamda*(Sc1*Sc1-Ss1*Ss1))*dsqrt(xmu-2d0*vv*Sp)&
             +2d0*xlamda*Ss2*Sc2*Ss1*Sc1*(xmu-2d0*vv*Sp)/xalpha
        ...
        a(4,2)=(1d0+xlamda*Sc1)*Ss1*Sc2*dsqrt(xmu-2d0*vv*Sp)&
             +xlamda*Ss1*Ss1*(Sc2*Sc2-Ss2*Ss2)*(xmu-2d0*vv*Sp)/&
              xalpha
